    ...
    <div class="product-outer">
        <div class="product-inner">
            @if ( $currency->category )
                <span class="loop-product-categories"><a href="/category/{{ $currency->category->id }}" rel="tag">{{ $currency->category->title }}</a></span>
            @endif
            <a href="/product/{{ $currency->id }}">
                <h3>
                    {{ $currency->title }}
                    {{-- @if($product->label)
                ...
                <div class="product-thumbnail">
                    <img src="/assets/images/blank.gif" data-echo="{{ $currency->image }}" class="img-responsive" alt="{{ $currency->title }}" />
                </div>
            </a>
