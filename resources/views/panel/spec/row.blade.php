							...
							<div class="form-wrap">
								<form action="@isset($row) {{ route('row.update', ['header' => $header->id, 'row' => $row->id]) }} @else {{ route('row.store', ['header' => $header->id]) }} @endisset" method="POST">
									...
									<div class="form-actions">
										<button class="btn @isset($row) btn-warning   custom-btn-warning @else btn-primary custom-btn-primary @endisset btn-icon right-icon mr-10 pull-left"> <i class="fa fa-check"></i>
									...
									<table id="datable_2" class="table table-hover table-bordered display mb-30" >
										<thead>
											<tr>
												...
												<th style="font-weight:bold; font-size:20px;">لیبل</th>
												<th style="font-weight:bold; font-size:20px;">مقادیر</th>
												<th style="font-weight:bold; font-size:20px;">راهنما</th>
												<th style="font-weight:bold; font-size:20px;">چند انتخاب</th>
										...
										<tbody>
											...
											<tr style="text-align:center;">
												...
												<td title="{{$item->help}}">{{ str_limit($item->help, 20) }}</td>
												...
												<td>
													<form action="{{ route('row.destroy', ['header' => $header->id, 'row' => $item->id]) }}" method="POST">
														<a href="{{ route('row.edit', ['header' => $header->id, 'row' => $item->id]) }}" class="btn btn-warning custom-btn-warning custom-pama-btn"><i class="icon ti-pencil"></i></a>
														<button type="submit" itemid="{{ $item->id }}" class="btn btn-danger custom-btn-danger custom-pama-btn delete-item"><i class="icon ti-close"></i></button>
